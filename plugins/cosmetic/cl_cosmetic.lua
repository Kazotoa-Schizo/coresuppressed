...
hook.Add("PostPlayerDraw", "impulseCosmeticDraw", function(k)
    ...
    if viewTFP and not toggleThirdPerson then
        if k == LocalPlayer() then
            return -- Skip drawing own cosmetics in third-person mode
        end
    end

    -- Draw cosmetics for other players
    if k.Cosmetics then
        for slot, cosmetic in pairs(k.Cosmetics) do
            if not IsValid(cosmetic) then continue end
            cosmetic:DrawModel()
        end
    end
