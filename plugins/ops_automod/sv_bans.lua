...
function impulse.Ops.AutoMod.Ban(ply, reason, risk, details)
    ...
    local ban_reason = "AutoMod ban for suspected "..reason..". Appeal @ https://citadel-community.vyhub.app/bans for review."

    if Vyhub then
        Vyhub:AddBan(ply:SteamID64(), 0, ban_reason, "0", Vyhub:CurrentTime(), function()
            Vyhub:InitBans()
        end)
    elseif VyHub then
    ...
    local embeds = {
        ...
        url = "https://citadel-community.vyhub.app/index.php?t=admin_bans&id="..ply:SteamID64(),
        color = 7774976,
        fields = {
            {
                name = "User",
