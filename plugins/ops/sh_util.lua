...
local setHealthCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        if targ and IsValid(targ) then
            targ:SetHealth(hp)
            targ:SetSyncVar(SYNC_BLEEDING,false,true)
...
local setArmorCommand = {
    ...
    onRun = function(ply, arg, rawText)
        ...
        if not ply:IsLeadAdmin() then
            armor = math.Clamp(armor, 1, 100)
        end

        if targ and IsValid(targ) then
            targ:SetArmor(armor)
            ply:Notify("You have set "..targ:Nick().."'s armor to "..armor..".")
        ...
        onRun = function(ply, arg, rawText)
            ...
            if plyTarget and ply != plyTarget then
                if plyTarget:IsSuperAdmin() then
                    return ply:Notify("This user can not be banned.")
                end
                ...
                local embeds = {
                    ...
                    url = "https://citadel-community.vyhub.app/index.php?t=admin_bans&id="..steamid,
                    color = 8801791,
                    fields = {
                        {
                            name = "User",
        ...
        onRun = function(ply, arg, rawText)
            ...
            if plyTarget and ply != plyTarget then           
                ...
                local embeds = {
                    ...
                    url = "https://citadel-community.vyhub.app/index.php?t=admin_warnings&id="..steamid,
                    color = 16774400,
                    fields = {
                        {
                            name = "User",
