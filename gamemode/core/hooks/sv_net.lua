...
util.AddNetworkString("impulseInvStorageLegalOpen")
...
util.AddNetworkString("impulseHL2RPVortEffect")
util.AddNetworkString("RoleplayAction")
util.AddNetworkString("chatBubble")
util.AddNetworkString("GetValidChatInfo")
util.AddNetworkString("ValidateChatInfo")
util.AddNetworkString("DynamicNotify")

local AUTH_FAILURE = "Invalid argument (rejoin to continue)"

net.Receive("ValidateChatInfo", function(len, ply)
    local sender = net.ReadEntity()
    local text = net.ReadString()
    local type = net.ReadUInt(8)

    net.Start("GetValidChatInfo")
...
net.Receive("impulseCharacterCreate", function(len, ply)
	...
	query:Callback(function(result)
        ...
        insertQuery:Insert("rpgroup", "0") -- rpgroup
...
net.Receive("impulseChatMessage", function(len, ply)
...
net.Receive("impulseInvDoMove", function(len, ply)
	...
	if ply.currentStorage:GetClass() == "impulse_storage_public" then
		...
		if item.Illegal then
			return ply:Notify("You may not access or store illegal items at City Storage Boxes.")
		end
	end
...
net.Receive("impulseDoConfiscate", function(len, ply)
    ...
    if item:GetPos():DistToSqr(ply:GetPos()) < (200 ^ 2) then
        ply.ConfiscatedItems = ply.ConfiscatedItems or {}
        table.insert(ply.ConfiscatedItems, {class = item.Item.UniqueID})

        if ply:Team() == TEAM_CP then
            if item.Item.Reward then
                local currentRankPoints = ply:GetRankPoints()
                if currentRankPoints < 1000 then
                    ply:Promote(item.Item.Reward, item.Item.Reward .. " Rank Points awarded for: Documentation and Removal of " .. itemName .. ".")
                else
                    ply:DynamicNotify("Confiscated: " .. itemName .. ".", 1)
                end
            else
                ply:DynamicNotify("Confiscated: " .. itemName .. ".", 1)
            end
...
net.Receive("impulseUnRestrain", function(len, ply)
	...
	ply:DynamicNotify("You have released "..ent:Name()..".", 1)
	ent:DynamicNotify("You have been released by "..ply:Name()..".", 1)

	ent:SetRunSpeed(impulse.Config.JogSpeed)
	ent:SetWalkSpeed(impulse.Config.WalkSpeed)
