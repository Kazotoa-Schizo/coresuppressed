	...
	if LocalPlayer():Team() != 0 and not vgui.CursorVisible() and not impulse_ActiveWorkbar then
		if not IsValid(impulse.MainMenu) or not impulse.MainMenu:IsVisible() then
			if input.IsKeyDown(KEY_F1) then
				...
				--if not ( cookie.GetString("impulse.changelogs"..impulse.Config.SchemaVersion.."seen", "false") == "true" ) then
					--Derma_Message("A new changelog has been published, go check it out!", "Changelogs", "Close")
				--end
...
function GM:ShouldDrawLocalPlayer()
	local headattachment = LocalPlayer():GetAttachment(LocalPlayer():LookupAttachment("eyes"))
...
function GM:CalcView(player, origin, angles, fov)
    ...
    local toggleThirdPerson = impulse.GetSetting("view_thirdperson", false)
    local viewTFP = LocalPlayer():EyePos()
    ...
    if viewTFP then
        local headattachment = player:GetAttachment(player:LookupAttachment("eyes"))

        if headattachment then
            local headBone = player:LookupBone("ValveBiped.Bip01_Head1") or player:LookupBone("ValveBiped.head") or player:LookupBone("ValveBiped.HC_Head_Bone")

            if headBone then
                player:ManipulateBoneScale(headBone, Vector(0, 0, 0))

                local pos = headattachment.Pos
                local angs = headattachment.Ang

                pos = pos + angs:Forward() * -1
	...
	if impulse.GetSetting("view_thirdperson") and player:GetViewEntity() == player then
		...
		local offset = Vector(5, 5, 5) -- make it so that this value can be altered in a clientside settingfor camera position

		offset.x = 30 -- 75 -- this is the forward/back of the camera
		offset.y = 12
		offset.z = 5
		angles.yaw = angles.yaw + 3

		local t = {} -- this sets up a line, might inject some bounding box stuff here, such as detecting if the box his anything. Another, possibly cheaper method, is to simply cast two more lines, on either side of the original. If either hit, the cam is moved out as if the central one hit.
		...
		tR.start = player:GetPos() + pos
		tR.endpos = tR.start + angles:Forward() * -offset.x

		tR.endpos = tR.endpos + angles:Right() * (offset.y - 5) -- added buffer
		tR.endpos = tR.endpos + angles:Up() * offset.z
		...
		tL.start = player:GetPos() + pos
		tL.endpos = tL.start + angles:Forward() * -offset.x

		tL.endpos = tL.endpos + angles:Right() * (offset.y + 5)
		tL.endpos = tL.endpos + angles:Up() * offset.z
		tL.filter = function(ent)
			if ent == LocalPlayer() then
				return false
			end
		...
		local trR = util.TraceLine(tR)
		local trL = util.TraceLine(tL)

		pos = tr.HitPos


		if (tr.Fraction < 1.0) then -- this is what makes the camera stick out from walls and stuff, works fine but if the cam is near the wall and not touching it you still see through.
			pos = pos + tr.HitNormal * 5
		elseif(trR.Fraction < 1.0) then
			pos = pos + trR.HitNormal * 5
		elseif(trL.Fraction < 1.0) then
			pos = pos + trL.HitNormal * 5
		end
