...
function GM:PlayerInitialSpawnLoaded(ply) -- called once player is full loaded
	...
	if Vyhub and Vyhub.Warnings[s64] then
		local bans = Vyhub:GetBans(s64)
		local warns = Vyhub.Warnings[s64]
...
function GM:PlayerSpawn(ply)
	...
	-- FIXME Lot of this code is written oddly (Nick)
...
--- Setups up userdata from Database data.
-- @internal
-- @realm server
-- @entity ply The player to see it.
-- @param dbData The database data to apply to the user.
function impulse.SetupPlayer(ply, dbData)
	...
	if (not ply:IsDonator() and userCount >= (impulse.Config.UserSlots or 9999)) then
		ply:Kick("The server is currently at full user capacity. Donate to the server for a reserved slot.")
		return
	end
	...
	if not Vyhub and (dbData.group and dbData.group != "user") then
	...
	-- ply:SetFOV(90, 0) -- this shit breaks users fov lol
	ply:SetTeam(impulse.Config.DefaultTeam)
...
function GM:PlayerSay(ply, text, teamChat, newChat)
	...
	if impulse.Ops then
		if impulse.Ops.AutoMod then
			if impulse.Ops.AutoMod.FilterMessage then
				local shouldBlock = impulse.Ops.AutoMod.FilterMessage(ply, text)
				if shouldBlock then
					return ""
				end
			end
	...
	if string.StartWith(text, "/") then
		...
		if command then
			...
			if command.adminOnly == true and not ply:IsAdmin() then
			...
			if command.communityManagerOnly == true and not ply:IsCommunityManager() then
				ply:Notify("You must be a community manager to use this command.")
				return ""
			end

			if command.superAdminOnly == true and not ply:IsSuperAdmin() then
...
-- Prevent r-con to bypass NLR timers for *those* kinds of people.
NLR_TABLE = NLR_TABLE or {}

function GM:PlayerDeath(ply, killer)
    -- Handle NLR timers
    local t = ply:Team()
    local sid = ply:SteamID64()
    NLR_TABLE[sid] = NLR_TABLE[sid] or {}
    if t != impulse.Config.DefaultTeam then
        NLR_TABLE[sid][t] = CurTime() + (impulse.Config.NewLifeTimer or 300)   
    end 
...
function GM:PlayerDeathThink(ply)
    if not ply.respawnWait then
        ply:Spawn()
        ply:AddSkillXP("vitality", -15 * ply:GetSkillLevel("vitality"))
        ply:Jail(60 + (ply.deathCount or 0) * 15) -- Jail the player with increased time
        ply.deathCount = math.min((ply.deathCount or 0) + 1, 9) -- Increment death count, but limit to 9

        -- Start a timer to decrement deathCount every 30 seconds
        timer.Create("DeathCountDecrement_" .. ply:SteamID64(), 600, 0, function()
            if ply.deathCount then
                ply.deathCount = math.max((ply.deathCount or 0) - 1, 0) -- Decrement death count, but limit to 0
            end
        end)
...
function GM:SetupPlayerVisibility(ply)
	-- Is there a nicer way to handle PVS? Perhaps write a module to add a PVS by a tag and remove the same way? (nick)
...
function GM:KeyPress(ply, key)
	...
	if key == IN_RELOAD then
		timer.Create("impulseRaiseWait"..ply:SteamID(), 0.3, 1, function()
			if IsValid(ply) then
...
function GM:GetFallDamage(ply, speed)
	...
	local strength = ply:GetSkillLevel("vitality")
...
impulse.BlacklistedVehicleModels = {
	["7seatvan"] = true,
	["8seattruck"] = true
}
...
function GM:PlayerSpawnVehicle(ply, model)
	...
	if ply:IsDonator() and (model:find("chair") or model:find("seat") or model:find("pod")) then
		if impulse.BlacklistedVehicleModels[model] then
			return ply:IsSuperAdmin()
		end
		local count = 0
...
function GM:SetupMove(ply, mvData)
	if isValid(ply.ArrestedDragging) then
		mvData:SetMaxClientSpeed(impulse.Config.JogSpeed - 30)
	end
end
...
function GM:PlayerShouldTakeDamage(ply, attacker)
	...
	if attacker and IsValid(attacker) and attacker:IsPlayer() and attacker != Entity(0) and attacker != ply then
		if (ply.NextStorage or 0) < CurTime() then
			ply.NextStorage = CurTime() + 5
		end

		attacker.NextStorage = CurTime() + 20
	end
