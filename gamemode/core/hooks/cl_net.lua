...
net.Receive("impulseNotify", function(len)
	...
	LocalPlayer():Notify(message,class)
end)
...
net.Receive("impulseZoneUpdate", function()
	...
	local zoneName = impulse.Config.Zones[zone] and impulse.Config.Zones[zone].name or "Unknown Zone"

    impulse.ShowZone = true
    LocalPlayer().impulseZone = zone
    --LocalPlayer():Notify("You have entered "..zoneName..".")
end)
...
net.Receive("impulseInvUpdateEquip", function()
	...
	if impulse_inventory and IsValid(impulse_inventory) then
		...
		impulse_inventory.modelPreview.setup = false
	end
end)
...
net.Receive("impulseInvStorageLegalOpen", function(len, ply)
	impulse_storage = vgui.Create("impulseInventoryStorageLegal")
end)
...
net.Receive("impulseConfiscateCheck", function()
	...
	if IsValid(item) then
		...
		--local request = Derma_Query("Confiscate "..item.HUDName.."?", 
			--"impulse",
			--"Confiscate",
			--function()
				--net.Start("impulseDoConfiscate")
...
net.Receive("chatBubble", function()
	...
	ReadChatInfo(player, text, type)
end)

net.Receive("GetValidChatInfo", function()
	local sender = net.ReadEntity()
	local text = net.ReadString()
	local type  = net.ReadUInt(8)

	GetValidChatInfo(sender, text, type)
end)

net.Receive("RoleplayAction", function()
	local meAction = net.ReadString()
	LocalPlayer():ConCommand(meAction)
end)