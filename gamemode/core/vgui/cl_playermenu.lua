...
function PANEL:QuickActions()
	...
	if impulse.IsHighRes() then btn:SetTall(30) btn:SetFont("Impulse-LightUI18") end
	btn:Dock(TOP)
...
function PANEL:Teams()
	...
	self.modelPreview:SetFOV(self.modelPreview:GetFOV() - 19)
 	...
 	self.descLbl:SetFont("Impulse-LightUI20")
...
function PANEL:Business()
	...
	for name,k in pairs(impulse.Business.Data) do
		...
		costLbl:SetFont(HIGH_RES("Impulse-LightUI20", "Impulse-LightUI20"))
...
function PANEL:Info()
	...
	for v,k in pairs(impulse.chatCommands) do
		...
		function command:Paint()
			draw.SimpleText(self.name, "Impulse-LightUI20", 5, 0, self.col)
			draw.SimpleText(self.desc, "Impulse-LightUI20", 5, 20, color_white)
			return true
		end
	end
...
function PANEL:AddSheet(name, icon, pnl, loadFunc)
	...
	function tab.Button:Paint(w, h)
		...
		draw.DrawText(name, HIGH_RES("Impulse-LightUI20", "Impulse-LightUI20"), (w-10)/2, 95, color_white, TEXT_ALIGN_CENTER)
