...
function PANEL:SetMix(mix)
    ...
    self.craftBtn:SetFont("Impulse-LightUI40")
...
function PANEL:RefreshCanCraft()
    ...
    self.canCraft = true
    local required = "<font=Impulse-LightUI32>"
...
local bodyCol = Color(50, 50, 50, 50)
...
function PANEL:Paint(w, h)
    ...
    if item then
        ...
        surface.SetFont("Impulse-LightUI40")
        local nameW, nameH = surface.GetTextSize(item.Name)
        surface.SetTextPos(82, -6)
        ...
        surface.SetFont("Impulse-LightUI40")
        surface.SetTextPos(nameW + 80, -6)
        surface.DrawText(" - Level: "..mix.Level)

        if self.requiredMarkup then
            self.requiredMarkup:Draw(82, h - 35, TEXT_ALIGN_LEFT, TEXT_ALIGN_TOP)
        end
    end
end
