...
function meta:CanBecomeTeam(teamID, notify)
    ...
    if teamData.whitelisted and not self:HasWhitelistLevel(teamData.name) then
        local add = teamData.whitelistFailMessage or ""
        if notify then self:Notify("You must be whitelisted to play as this team. "..add) end
        return false
    end
...
function meta:CanBecomeTeamClass(classID, notify)
    ...
    if ( classData.xp ) then
        if ( self:GetXP() < classData.xp ) then
            if notify then self:Notify("You don't have the XP required to play as this class.") end
            return false
        end
    end
    
    if ( classData.whitelisted ) then
		if not ( self:HasWhitelistLevel(classData.name) ) then
...
function meta:CanBecomeTeamRank(rankID, notify)
    ...
    if ( rankData.xp ) then
        if ( self:GetXP() < rankData.xp ) then
            if notify then self:Notify("You don't have the XP required to play as this rank.") end
            return false
        end
    end
    
    if ( rankData.whitelisted ) then
		if not ( self:HasWhitelistLevel(rankData.name) ) then
