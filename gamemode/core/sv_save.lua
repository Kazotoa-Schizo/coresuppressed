...
concommand.Add("impulse_save_saveall", function(ply, cmd, args)
	...
	for v,k in pairs(ents.GetAll()) do
		if k.impulseSaveEnt then
			table.insert(savedEnts, {pos =  k.AlwaysPos or k:GetPos(), angle = k:GetAngles(), class = k:GetClass(), model = k:GetModel(), keyvalue = (k.impulseSaveKeyValue or nil), parent = (k:GetParent() and k:GetParent())})
		end
	end

	file.Write("impulse/saves/"..string.lower(game.GetMap())..".dat", util.TableToJSON(savedEnts, true))
