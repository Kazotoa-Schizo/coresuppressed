...
function meta:CanHoldItem(itemclass, amount)
	...
	if self:IsTeamCP() then
		return self.InventoryWeight + weight <= impulse.Config.InventoryCPMaxWeight
	elseif self:IsTeamOTA() then
		return self.InventoryWeight + weight <= impulse.Config.InventoryOTAMaxWeight
	elseif self:IsTeamVort() then
		return self.InventoryWeight + weight <= impulse.Config.InventoryVortMaxWeight
	else
...
function meta:DropInventoryItem(itemid)
	...
	--if self.DroppedItemsC > 1 and ((self.NextItemDrop or 0) > CurTime() or self.DroppedItemsC > 1) then -- prevents lag
