...
function SWEP:AllowPickup(target)
	...
	if ( self.Owner:Team() == TEAM_CITIZEN and self.Owner:GetTeamClass() == CLASS_CWU_INDUSTRIAL ) then
		if ( target:GetClass() == "impulse_hl2rp_cloth" or target:GetClass() == "impulse_hl2rp_delivery_crate" ) then 
			return true 
		end
	end

	if ( target:GetClass() == "impulse_scanner" ) then 
		return false 
	end
